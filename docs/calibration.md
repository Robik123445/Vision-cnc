...
## Calibration artifacts metadata
Saved JSON now includes `meta` fields (`schema_version`, `created_at`, `camera_id`, `frame_checksum`) and quality metrics (`reprojection_error`, `rmse_mm`) pre bezpečný audit v produkcii.
